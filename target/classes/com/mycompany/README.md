...
- **Entities (`entity/`)**: Represents the core data models.
  - `Book`: Contains details like ID, title, author, genre, year, quantity, and borrow count.
  - `Member`: Represents library users (Regular or Premium types), storing their information, current borrow count, and total borrowing history.
  - `BorrowRecord`: Tracks borrowing transactions, linking a member, a book, borrow date, and return status.
